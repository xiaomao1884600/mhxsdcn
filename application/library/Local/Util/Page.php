...
class Page
{
	...
	public static function displayFormElement($name, $value, $type = 0)
	{
		...
		switch ($type)
		{
			...
			case 3 :
				break;
			case 4 :
				$formElement .= "<textarea class=\"form-control\" rows=\"3\" name=\"{$name}\" style=\"resize:none;\">{$value}</textarea>";
				break;
			default :
	 ...
	 * 文件上传状态验证
	 * */
	public static function checkUploadStatus($uploadStatus)
	{
		if ($uploadStatus)
		{
			switch ($uploadStatus)
			{
				case 1:
					Local\Util\Page::displayError('上传文件的大小超过限制');
					break;
				case 2:
					Local\Util\Page::displayError('上传的文件扩展名是不正确的');
					break;
				case 3:
					Local\Util\Page::displayError('非法提交');
					break;
				default :
					Local\Util\Page::displayError('文件上传失败');
					break;
			}
		}
	}
	 ...
	 * @param string $type 图片类型(原图、缩略图$type=thumb、展示图$type=show)
	 * @param string $extension 资源类型
	 * @return string
	 */
	public static function getImageCacheURL($filepath, $type = '', $extension)
	{
	
		if (file_exists(\Yaf\Registry::get('config')->classesmobile->attachment . $filepath) && $filepath)
		{
			return 'http://img' . rand(0, 9) . \Yaf\Registry::get('config')->application->picUrl . ($type ? ('/' . $type) : '') . $filepath;
		}
		else
		{
			return 'http://img' . rand(0, 9) . \Yaf\Registry::get('config')->application->picUrl . '/default/' . $extension . '.gif';
		}
	}
